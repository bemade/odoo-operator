...
async fn image_pull_secret_copied_to_namespace() {
    ...
    // Wait for the operator to copy the secret into the test namespace.
    let ns_secrets: Api<Secret> = Api::namespaced(c.clone(), ns);
    assert!(
        wait_for(TIMEOUT, POLL, || {
            let ns_secrets = ns_secrets.clone();
            async move { ns_secrets.get("test-registry").await.is_ok() }
        })
        .await,
        "expected registry secret to be copied into instance namespace"
    );

    // Verify the type was preserved.
    let copied = ns_secrets.get("test-registry").await.unwrap();
    assert_eq!(
        copied.type_.as_deref(),
        Some("kubernetes.io/dockerconfigjson"),
        "secret type should be preserved"
    );
}
